...
# Physical velocity 40km/h = 11.11m/s
VELOCITY = 0.0110
...
# Grid size (unit: meters)
GRID_SIZE = 500  # 500
# Maximum grid number on the X-axis
GRID_XNUM = int(VERTICAL_LENGTH / GRID_SIZE) + 1
# Maximum grid number on the Y-axis
...
if __name__ == "__main__":
    ...
    exit()  # Exiting here for testing purposes; remove this line to execute the following code
    
    mapStart = time.perf_counter()
    mapContext = ServletContext()
    g = GraphUtils()
    g.saxBigGraphImport("..\\..\\data\\map_NewYork", mapContext)
    print('Parsing map took', time.perf_counter() - mapStart)

    saveStart = time.perf_counter()
    file = open('..\\..\\data\\mapContext_NY.dat', 'wb')
    pickle.dump(mapContext, file)
    file.close()
    print('Saving map took', time.perf_counter() - saveStart)

    readStart = time.perf_counter()
    file = open('..\\..\\data\\mapContext_NY.dat', 'rb')
    mapContext = pickle.load(file)
    print(len(mapContext.nList), len(mapContext.eList))
    print('Reading map took', time.perf_counter() - readStart, 's')
